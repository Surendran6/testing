...
function ShortnerForm() {
  ...
  const validationSchema = Yup.object().shape({
    longurl: Yup.string().url("Enter valid URL").required("Enter URL!"),
  });
    ...
    try {
      if (token) {
        const result = await axios.post(
          `http://localhost:5000/api/private/shorten`,
          { longURL: longurl },
          {
            headers: {
          ...
          <Form>
            <div className="form-row align-items-center">
              ...
              <div className="col-auto my-1">
                <button type="submit" className="btn btn-primary align-button">
                  Submit
                </button>
              </div>
            </div>
