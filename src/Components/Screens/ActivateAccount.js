  ...
  const Activate = async () => {
    try {
      const result = await axios.get(
        `http://localhost:5000/api/auth/activate-account/${token.token}`
      );
  ...
  return (
    <>
      ...
      <div className="container-fluid center-content mt-3">
        <h4 style={{ color: "rgb(86, 3, 88)", marginBottom: "20px" }}>
          Thanks for your time and interest in registering to our application!
        </h4>
        {!activate && token ? (
          <>
